...
## Atualização 21/08/2026 — Under25: sessão sob demanda + fix liquidez + telemetria + gap crítico em `apostas` (EM ANDAMENTO)

- **Contexto**: usuário levantou hipótese de que o bloqueio de conta de ~23h (13/08) é um limite de "jogo seguro"/tempo de sessão contínuo da própria Betfair, não do token de API — confirmado por ele diretamente na conta (não é hipótese, é fato: "confirmei por padrão 23 horas"). Keep-alive (que só renova o token) não resolve isso; a solução é não deixar a conta logada continuamente, principalmente no Under25 (que ficava logado o dia inteiro rodando `while True` com polling a cada 60s).

- **Fix 1 — Under25 em modo "sob demanda"** (`bot_under25.py`, backup `bot_under25.py.bak_janelas_*`): reescrito `main()` para calcular janelas de atividade a partir dos kickoffs dos próximos jogos elegíveis (`buscar_kickoffs_futuros()` + `construir_janelas()`, fundindo blocos sobrepostos com folga de `ENTRADA_MINUTOS_MAX + SAIDA_MINUTOS + 2min`, usando os filtros reais do Supabase). Bot só loga durante uma janela ativa ou com aposta aberta; desloga (`bf.logout()`) fora disso. Janelas recalculadas a cada 2h. Sleep de 60s quando ativo, 300s quando ocioso. Validado em produção: log mostrou `Janelas atualizadas: 3 bloco(s)` seguido de `Fora de janela ativa e sem apostas abertas - sessao encerrada` — comportamento correto confirmado.
  - **Pendente**: observar por alguns dias via `v_status_sessao_betfair` se isso realmente evita o padrão de bloqueio de ~23h (a causa raiz é uma trava de conta da Betfair, não um bug de código — não há garantia total até um ciclo completo passar sem bloqueio).

- **Fix 2 — bug de liquidez no Under25** (`bot_under25.py`, backup `bot_under25.py.bak_liquidez_*`): usuário subiu `LIQUIDEZ_MINIMA` de 150 para 20000 em 19/08, mas um cap hardcoded em `verificar_entradas()` (`min(50, liq_minima)` pro minuto ≤2, `min(100, liq_minima)` pro minuto ≤4) sobrescrevia esse valor sempre que a entrada acontecia nos minutos 1-4 — ou seja, praticamente sempre, já que `ENTRADA_MINUTOS_MAX=5`. Na prática o filtro real sendo aplicado era £50-100, não £20000. Confirmado nos dados: as últimas 20 aprovações tinham `liquidez_disponivel=0` (ver Fix 3) e minutos 1-3. Cap removido; `LIQUIDEZ_MINIMA` do Supabase agora vale sempre, sem exceção por minuto.

- **Fix 3 — telemetria de liquidez** (mesmo arquivo/patch do Fix 2): `verificar_entradas()` nunca passava o campo `liquidez_disponivel` nas chamadas de `sb.registrar_analise_supabase()` (nem em aprovações nem em reprovações por odd/liquidez) — a coluna existe na tabela `analises` e a função já sabia gravá-la (`info.get('liquidez_disponivel', 0)`), só não recebia o dado. Corrigido: as 3 chamadas (aprovado, reprovado por odd, reprovado por liquidez) agora passam `"liquidez_disponivel": liq`. Sem isso, `liquidez_disponivel` sempre gravava `0`, mascarando o bug do Fix 2 no dashboard/histórico.

- **Levantamento (14 dias, Under25, tabela `analises`)**: 5.290 reprovações vs 586 aprovações (~10% taxa de aprovação). 89% das reprovações são por odd fora de [1.8, 2.0] (5.266), só 0,4% por liquidez (23 — baixo porque o cap do Fix 2 ainda não tinha sido corrigido nesse período). Das reprovações por odd, ~53% (2.774) estão coladas na borda do intervalo (1.50-1.79 ou 2.01-2.30) — candidato a testar alargamento do intervalo, mas falta dado de resultado real pra decidir.

- **Achado crítico (BLOQUEIA o backtest de odd pedido pelo usuário)**: tabela `apostas` está **vazia** para o Under25 (`bot_id=4101d27c...`) — zero linhas, apesar de `analises` ter 586 aprovações nos últimos 14 dias. Causa: `bot_under25.py` nunca chama `sb.registrar_aposta_supabase()` (o INSERT que cria a linha em `apostas` no momento da entrada) — só a função de análise (`registrar_analise_supabase`) e, na saída, `atualizar_resultado_aposta_supabase()`, que faz um UPDATE filtrado por `status='PENDENTE'` sem nunca encontrar linha pra atualizar (mesmo padrão de "log de sucesso sem confirmar que a operação afetou algo" já documentado antes nesse arquivo, mas em bug novo/lugar novo). UPDATE de 0 linhas não gera exceção, então passa silencioso. Resultado: Under25 nunca aparece no dashboard/`apostas` como o LAY aparece, e não há como fazer backtest de odd por falta de resultado real (só existe "aprovado/reprovado", nunca "ganhou/perdeu com PnL").
  - **Pendente (próxima sessão)**: adicionar chamada a `sb.registrar_aposta_supabase()` em `verificar_entradas()` no momento da aprovação, adaptando os campos genéricos da tabela `apostas` (que foi desenhada em torno do mercado Correct Score do LAY — `odd_lay`/`placar_lay`) pro contexto do Under 2.5: usar `odd_lay` pra guardar a odd de entrada Under 2.5, deixar `placar_lay` vazio/null, `market_id` = market_id do mercado Under 2.5. Preciso ver `_validar_aposta()` (ainda não visto) pra saber quais campos são obrigatórios antes de escrever o patch. Só depois desse fix dá pra rodar o backtest de win-rate-por-faixa-de-odd que o usuário pediu (ideia: expandir [1.8, 2.0] pra algo como [1.7, 2.2] se o win rate nas bordas próximas se sustentar).

*Ultima atualizacao: 21/08/2026*

## Atualização 22/08/2026 — LAY: sessão sob demanda (logout ocioso) + limpeza de monitores + fix "IA indisponível" + dedupe do watchdog

- **Contexto**: usuário confirmou que o bloqueio de conta de ~23h é um limite de sessão contínua da própria Betfair (fato confirmado por ele diretamente na conta, não hipótese). O keep-alive proativo (fix de 19/08) resolvia sessão *expirando por token vencido*, mas mantinha a sessão viva o dia inteiro — não resolve o limite real. Também confirmado que o bot não faz cash-out antecipado (aposta é mantida até o resultado final), então o monitoramento contínuo de odds pós-aprovação não tem função prática.

- **Fix 1 — sessão sob demanda no LAY** (`bot_prelive.py`, `rodar_bot()`): removido o `bf.renovar_token_se_necessario()` fixo no topo do loop (fix de 19/08, revertido — mantinha sessão viva o dia todo). Login voltou a ser reativo via `chamar_api()` (já loga sozinho quando não há sessão válida). Adicionado cálculo de "próximo evento necessário" no trecho ocioso do loop: olha a fila de jogos aguardando, a próxima recarga (`INTERVALO_RECARGA_HORAS`) e a próxima checagem de resultado (`INTERVALO_RESULTADO_MIN`); se o mais próximo estiver a mais de `MARGEM_LOGOUT_MIN=15min` de distância, chama `bf.logout()` (no-op barato se já deslogado). Nova constante `MARGEM_LOGOUT_MIN=15` adicionada perto de `INTERVALO_LONGE`.
  - Validado em produção 21/08 ~21h: log limpo, sem erro. Em horário de agenda cheia (jogo a cada 15-30min), o logout praticamente não dispara (esperado — sempre tem algo por perto). Efeito real esperado nas janelas de baixo movimento (madrugada, ~3h-8h Brasília) — pendente de observação via log/`v_status_sessao_betfair`.
  - Backup: `bot_prelive.py.bak_logout_ocioso_20260821_210202`.

- **Fix 2 — remoção do monitor de odds/saída pós-aprovação**: `monitor_odds`/`monitor_saida` (classes `MonitorOdds`/`MonitorSaida`, só emitiam alertas informativos no Telegram sobre movimento de odds, nunca executavam saída automática) pararam de ser alimentados (`monitor_odds.adicionar(info)` / `monitor_saida.adicionar(info)` removidos) e de ser verificados no loop (`monitor_odds.verificar_todos()` / `monitor_saida.verificar_todos()` removidos). Resultado final da aposta continua sendo resolvido normalmente via `resultado_jogos.py` (rotina separada, já existente).
  - Limpeza cosmética: linha de status do loop (`agendador.status()`) parou de mostrar `Monitor odds: 0 | Monitor saida: 0` (sempre ficaria 0 agora). Mensagem de start no Telegram trocou `📡 Monitor de odds e saída: *ativo*` por `🔌 Sessão Betfair: sob demanda (desloga se ocioso >{MARGEM_LOGOUT_MIN}min)`. Linha do corpo do alerta de aprovação trocou `📡 _Monitorando odds e saída automaticamente_{ia_str}` por só `{ia_str}` (texto não era mais verdade).

- **Fix 3 — bug do filtro "IA indisponível" que vazava pro Telegram**: comparação em `formatar_alerta()` (linha `ia_str`) checava igualdade exata com o texto literal `'IA indisponível'`, mas as mensagens reais geradas por `consultar_ia()` são `"IA indisponivel (HTTP {codigo})"`, `"IA indisponivel (resposta invalida/truncada)"`, `"IA indisponivel ({tipo_erro})"` ou `"IA indisponivel (limite de tentativas)"` — sem acento e sempre com sufixo entre parênteses. A comparação nunca batia (bug duplo: acento + sufixo), então a frase sempre aparecia no alerta do Telegram sempre que a IA falhava ou estava indisponível. Corrigido para `not info['ia_motivo'].startswith('IA indisponivel')` (prefixo, sem depender de acento).
  - Nota tecnica: a causa do anchor de patch ter falhado na primeira tentativa foi a mesma raiz — o "í" de "disponível" no arquivo real da VPS está numa forma de normalização Unicode diferente da digitada (visualmente idêntico, byte a byte diferente). Patch final usou regex casando só pelo prefixo ASCII estável da linha, evitando esse problema.
  - Backup: `bot_prelive.py.bak_ia_monitor_cleanup_20260822_033002`.

- **Fix 4 — dedupe de alertas repetidos do watchdog** (`watchdog_bot.sh`): causa raiz real do problema já visto em 11/08 (9 alertas seguidos, na época só paliado subindo `LIMITE_SEM_ANALISE_MIN` de 90 para 180min) identificada: quando a fila de jogos fica vazia por muitas horas (madrugada), o watchdog reinicia o serviço E alerta no Telegram a CADA ciclo de cron (5 em 5min) enquanto a lacuna sem `POST /analises` segue crescendo — reiniciar não cria jogo novo pra analisar, então o mesmo incidente gerava uma rajada de mensagens idênticas.
  - Fix: novo `STATE_FILE` (`.watchdog_silencioso_ts`) grava o timestamp da última análise (`ultima_analise_ts`) já tratada. Só alerta/reinicia de novo se uma análise nova tiver acontecido desde então (incidente genuinamente diferente); caso contrário só loga "já tratado" sem spam. Estado é limpo (`rm -f`) quando a checagem volta a ficar saudável, preparando pro próximo incidente real.
  - Validado com `bash -n`. Não precisa restart de serviço (roda via cron, efeito no próximo ciclo). Backup: `watchdog_bot.sh.bak_dedupe_<timestamp>`.
  - **Pendente**: confirmar amanhã via `tail -50 watchdog.log` que, numa madrugada com fila vazia por 180+min, só dispara UM alerta (não uma rajada).

*Ultima atualizacao: 22/08/2026*

## Atualização 22/08/2026 (cont.) — Janela diária de descanso forçado (02h-05h) nos dois bots

- **Motivação**: mesmo com os dois bots já fazendo logout por ociosidade (LAY via cálculo de folga, Under25 via janelas de kickoff desde 21/08), o limite de ~23h da Betfair é provavelmente por conta, não por token de sessão — se as janelas ociosas de cada bot não coincidirem no tempo, a conta nunca fica de fato sem nenhuma sessão aberta, e o risco de bloqueio persiste. Decisão: em vez de fusão total dos bots ou sessão/token compartilhado entre os dois processos (avaliados e descartados por risco/complexidade desproporcional ao ganho), implementar uma janela diária fixa e simultânea de descanso forçado nos dois, sem precisar os processos "conversarem" entre si.
- **Janela definida**: 02h-05h (Brasília), horário de menor movimento de jogos elegíveis pros dois bots.
- **LAY** (`bot_prelive.py`): novas constantes `HORA_DESCANSO_INICIO=2` / `HORA_DESCANSO_FIM=5`. No ramo ocioso do loop (já existente, do fix de logout por folga de hoje mais cedo), adicionado `em_janela_descanso` — se a hora Brasília atual cair na janela, força `bf.logout()` independente do cálculo normal de folga (`MARGEM_LOGOUT_MIN`). Só se aplica quando já estamos no ramo "sem jogo pra verificar agora" — não pula uma verificação real que porventura caia dentro da janela (jogo tardio raro). Log distingue `sessao Betfair encerrada (janela de descanso 02h-05h)` do logout normal por folga.
- **Under25** (`bot_under25.py`): mesmas constantes. Antes do `if ativo:`, adicionado: se estiver na janela de descanso E não houver aposta aberta (`len(apostas_ativas) == 0`), força `ativo = False` — isso já aciona o `bf.logout()` que existe no ramo `else` (lógica de janela sob demanda de 21/08, sem alteração). Trava de segurança explícita: **nunca** força o descanso se houver aposta aberta sendo monitorada (`verificar_saidas` não pode ser interrompido). Log distingue `Janela de descanso forcado (02h-05h) - sessao encerrada` do logout normal fora de janela de jogo.
- Validado com `py_compile` (LAY) antes de aplicar; patch do Under25 só pôde ser testado sintaticamente isolado (não havia cópia completa do arquivo disponível fora da VPS), aplicado com script anchor-based que aborta sozinho se o trecho real não bater — sem risco de aplicação parcial.
- Backups: `bot_prelive.py.bak_descanso_forcado_<timestamp>`, `bot_under25.py.bak_descanso_forcado_<timestamp>`.
